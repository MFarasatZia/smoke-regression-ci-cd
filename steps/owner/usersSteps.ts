...
Then("Verify user full name on Users Page", async function () {
	...
	assert(
		expectedFullName.toLowerCase() == actualFullName.toLowerCase(),
		"Expected: " + expectedFullName + " Actual: " + actualFullName,
	);
});

Then("Verify user {string} on Users Page", async function (fullname: string) {
	const actualFullName = await userspage.getFullNameText();
	assert(
		fullname.toLowerCase() == actualFullName.toLowerCase(),
		"Expected: " + fullname + " Actual: " + actualFullName,
	);
});
